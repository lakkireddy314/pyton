...
def certAsDict( cert ) :
  ...
  if type( cert ) == type( '' ) or type( cert ) == type( '' ) :
    ...
    if dict :
      ...
      for name in names :
        ...
        if type( value ) == type( '' ) or type( value ) == type( '' ) :
          ...
          print('%*s : {' % ( width, name ))
          displayDict( value, width + 4 + max( [ len( x ) for x in list(value.keys()) ] ) )
          print('%*s   }' % ( width, ' ' ))
        else :
...
def getPorts( serverName, nodeName = None ) :
  ...
  for name, port in list(endPoints.items()) :
    result[ port ] = name
  return result
...
#   import Help, AdminControl          # Get access to WSAS objects
    ...
    if value.endswith( notavail ) :
      if 'AdminTask' in list(sys.modules.keys()) :
